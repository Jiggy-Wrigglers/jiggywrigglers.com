    ...
    <div class="wrap">
        <div class="register-interest-content">
            <?php if ( $register_heading ) : ?>
                <h3 class="heading-5"><?php echo wp_kses_post( $register_heading ); ?></h3>
            <?php endif; ?>
            <?php if ( $register_title ) : ?>
                <h2 class="heading-2"><?php echo wp_kses_post( $register_title ); ?></h2>
            <?php endif; ?>
            <?php if ( $register_text ) : ?>
                <p class="body-medium register-interest-text"><?php echo wp_kses_post( $register_text ); ?></p>
            <?php endif; ?>
            <?php if ( $register_button ) : ?>
                <a class="button button-orange" href="<?php echo esc_url( $register_button['url'] ); ?>"<?php echo ! empty( $register_button['target'] ) ? ' target="' . esc_attr( $register_button['target'] ) . '"' : ''; ?>>
                    <?php echo esc_html( $register_button['title'] ); ?>
                </a>
            <?php endif; ?>
        </div>
        <?php if ( $register_image ) : ?>
            <div class="register-interest-image">
